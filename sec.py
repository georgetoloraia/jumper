...
class Secp256k1:
    ...
    def scalar_mult(k, point):
        ...
        # print(f"Starting scalar multiplication with private key k = {k}")
        while k:
            if k & 1:
                result = Secp256k1.point_add(result, addend)
                steps.append(f"Added point: ({result.x}, {result.y})")
            addend = Secp256k1.point_add(addend, addend)
            steps.append(f"Doubled point: ({addend.x}, {addend.y})")
            k >>= 1
...
# Example usage:
# private_key = 0x4f2d0e3d821f315bcf44d95f033876c72425f604e05f7dd027869c13f881d737  # This should be a large, random number in a real application
